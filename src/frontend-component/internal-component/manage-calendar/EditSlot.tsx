...
import { manageSingleSlotsQuery, manageSlotsQuery } from "src/hook/query/slot";
import { manageSlotUpdateMutation } from "src/hook/mutations/manage-slot";
function EditSlot() {
  const classes = useStyles();
  const authKey = localStorage.getItem("TOKEN");
  const navigate = useNavigate();
  const params = useParams();
  const [slotMutate, { loading }] = useMutation(manageSlotUpdateMutation, {
    context: {
      headers: {
  ...
  const { loading: singleLoading, data } = useQuery(manageSingleSlotsQuery, {
    variables: {
      findSlotId: params?.id,
    },
    context: {
      headers: {
        Authorization: `Bearer ${authKey}`,
  ...
  const {
    ...
    initialValues: {
      startTime: data?.findSlot?.startTime,
      endTime: data?.findSlot?.endTime,
      reservationDate: data?.findSlot?.startDate,
    },
  ...
  if (loading || queryLoading || singleLoading) {
